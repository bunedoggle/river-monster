...
CSI::CSI(HardwareSerial* serial_ptr) {
...
CSI::CSI(HardwareSerial* serial_ptr, SoftModem* m_ptr) {
...
CSI::CSI(HardwareSerial* serial_ptr, SoftModem* m_ptr, HardwareSerial* debug_ptr) {
...
char* CSI::getInt(char* cmd, int* data){
  ...
  while(cmd[i] != END_OF_FRAME && cmd[i] != ','){
    d *= 10;
    d = ((cmd[i])-48)+ d;
    i++;
  }
